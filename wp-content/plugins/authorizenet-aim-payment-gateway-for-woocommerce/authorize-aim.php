      ...
      **/
      function payment_fields()
      {
         if ( $this->description ) 
			...
			echo '<style>li.payment_method_authorizeaim > input#payment_method_authorizeaim{display:none;}</style>';
       ...
       $response_array = explode('|',$post_response);
         ...
         if ( count($response_array) > 1 ){
            ...
            if($response_array[0] == '1' ){
				...
				$notice='Transaction Error : '.$response_array[3];
			    wc_add_notice( $notice, 'error' );
            ...
            $order->update_status('failed');
             $order->add_order_note($this->failed_message .$response_array[3] );
				$notice='Error processing payment.';
				wc_add_notice( $notice, 'error' );
