<?php
/*
Plugin Name: WooCommerce add meta field in product
Description: This is a woo-commerce plugins to add meta data in the product and price will
will be added in the cart.
Version: 1.1
Author: Sawan Kumar
		...
		class woocommerce_product_addons {
		    ...
		    function meta_box( $post ) {
				...
				<div id="product_addons" class="panel">
					...
					<h4>
						<a href="#" class="import tips" tip="<?php _e('Import', 'wc_product_addons'); ?>"><?php _e('Import', 'wc_product_addons'); ?></a>
						<a href="#" class="export tips" tip="<?php _e('Export', 'wc_product_addons'); ?>"><?php _e('Export', 'wc_product_addons'); ?></a>
			...
			function order_item_meta( $item_id, $cart_item ) {
			     //woocommerce_add_order_item_meta( $item_id, 'Olho Direito', 'kjkjkj' );
				// Add the fields
			...
			function validate_add_cart_item( $passed, $product_id, $qty ) {
				...
				if (is_array($product_addons) && sizeof($product_addons)>0) foreach ($product_addons as $addon) {
					...
					if ($addon['required']) {
						...
						if (!$passed) {
							 $notice = sprintf( __('"%s" is a required field.', 'woocommerce'), $addon['name']);
